...
import { Edit3, Lock, Trophy, X, RefreshCw } from 'lucide-react';
...
export default function BracketCard({
  ...
  const renderPredictionChoice = () => {
    ...
    if (choice === 'home') {
      return (
        <span className="inline-flex items-center gap-1 bg-primary/10 border border-primary/20 text-primary px-1.5 py-0.5 rounded-lg text-[10px] font-bold min-w-0 max-w-[105px] shadow-sm shadow-primary/5">
          <img src={match.home_logo} className="h-3 w-4.5 object-cover rounded-sm bg-white/5 flex-shrink-0" alt="" />
          <TeamName 
            name={match.home_team} 
            className="text-[10px] font-bold text-primary truncate max-w-[65px]" 
            spanClassName="max-w-[65px] truncate block"
          />
        </span>
      );
    }
    ...
    return (
      <span className="inline-flex items-center gap-1.5 bg-white/5 border border-white/10 text-white px-1.5 py-0.5 rounded-lg text-[10px] font-bold shadow-sm">
        Hòa
      </span>
    );
  };
  ...
  const getPointsBadge = (points: number | null) => {
    ...
    if (points === 1) {
      return (
        <span className="bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-[8px] font-extrabold px-1 py-0.5 rounded flex-shrink-0 ml-0.5 shadow-sm shadow-emerald-500/5">
          +1đ
        </span>
      );
    }
    if (points > 0) {
      return (
        <span className="bg-amber-500/20 border border-amber-500/30 text-amber-400 text-[8px] font-extrabold px-1 py-0.5 rounded flex-shrink-0 ml-0.5 shadow-sm shadow-amber-500/5">
          +{points}đ
        </span>
      );
    }
    return (
      <span className="bg-white/5 border border-white/10 text-muted-foreground text-[8px] font-extrabold px-1 py-0.5 rounded flex-shrink-0 ml-0.5">
        0đ
      </span>
    );
  };
  ...
  return (
    <div
      ...
      className={`w-60 h-24 bg-white/[0.02] border rounded-xl p-2 transition-all text-left flex flex-col justify-between select-none relative group ${
    ...
    >
      {/* Mini info overlay */}
      <div className="flex items-center justify-between text-[9px] font-bold text-muted-foreground uppercase tracking-wider mb-1 border-b border-white/5 pb-0.5">
      ...
      {match && (
        <div className="mt-1 pt-0.5 border-t border-white/[0.03] flex items-center justify-between min-h-[16px] gap-2">
          <div className="flex items-center min-w-0 flex-1">
            {userPrediction ? (
              <div className="flex items-center gap-1.5 text-[10px] font-bold min-w-0 flex-1 truncate">
              ...
              <span className="flex items-center gap-1 italic text-muted-foreground/50 text-[10px] flex-shrink-0">
                <Lock className="h-2.5 w-2.5" /> Khóa
              </span>
            ) : (
              // Bỏ dòng chữ "Nhấp để dự đoán" rườm rà đối với các trận đấu chưa dự đoán & chưa khóa
              <span className="text-[10px] text-muted-foreground/0 select-none">&nbsp;</span>
            )}
          </div>
          ...
          {isAdmin && (
            <button
              ...
              className="p-1 rounded bg-white/5 border border-white/10 text-muted-foreground/80 hover:bg-white/10 hover:text-white active:scale-95 transition-all cursor-pointer flex items-center justify-center flex-shrink-0"
