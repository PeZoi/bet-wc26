      ...
      <div className="flex flex-col items-center justify-center py-20 text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground max-w-[45ch]">
            Vui lòng đăng nhập bằng tài khoản Google để tham gia dự đoán tỉ số và theo dõi thứ hạng của mình.
          </p>
        </div>
      </div>
    );
        ...
        <div className="glass-panel rounded-3xl p-6 border border-white/5 flex flex-col justify-between space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-muted-foreground font-medium mt-0.5">Trúng tỉ số</span>
            </div>
            <div className="flex flex-col">
              <span className="text-2xl font-extrabold font-mono text-cyan-400">
                {userProfile?.correct_outcomes_count || 0}
              </span>
              <span className="text-[10px] text-muted-foreground font-medium mt-0.5">Trúng kết quả</span>
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel rounded-3xl p-6 border border-white/5 flex flex-col justify-between space-y-4">
          ...
          {nextMatch && (
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 rounded-xl bg-primary text-xs font-bold text-primary-foreground hover:bg-primary/90 py-2 transition-all cursor-pointer"
            >
              Dự đoán ngay
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {unpredictedMatches.map((match) => (
                <div key={match.id} className="bg-white/5 rounded-2xl p-4 border border-white/5 flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between text-xs py-1.5 gap-1">
                    <div className="flex items-center gap-1.5 min-w-0 flex-1">
                      ...
                      />
                      <span className="font-bold text-white truncate">{match.home_team}</span>
                    </div>
                    <span className="text-muted-foreground font-mono px-1 flex-shrink-0">vs</span>
                    <div className="flex items-center gap-1.5 min-w-0 flex-1 justify-end">
                      <span className="font-bold text-white truncate">{match.away_team}</span>
                      <img
                        src={match.away_logo}
                        alt=""
                        className="h-3.5 w-5 object-cover rounded-sm bg-white/5 flex-shrink-0"
              ...
              {userRecentPredictions.map((match) => {
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col gap-1 min-w-0">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <div className="flex items-center gap-1 min-w-0">
                          ...
                          />
                          <span className="font-bold text-white truncate max-w-[55px]">{match.home_team}</span>
                        </div>
                        <span className="text-[10px] text-muted-foreground font-mono flex-shrink-0">vs</span>
                        <div className="flex items-center gap-1 min-w-0">
                          <span className="font-bold text-white truncate max-w-[55px]">{match.away_team}</span>
                          <img
                            src={match.away_logo}
                            alt=""
                            className="h-3 w-4 object-cover rounded-sm bg-white/5 flex-shrink-0"
                    ...
                    {pred && (
                      <div className="flex items-center gap-2">
                        <span className="font-mono bg-white/5 border border-white/5 px-2 py-0.5 rounded text-white font-bold">
                          {pred.predicted_home_score} - {pred.predicted_away_score}
                        </span>
                        {match.status === 'FT' && (
                          <span className={`font-mono font-bold ${pred.points_earned === 3 ? 'text-emerald-400' : pred.points_earned === 1 ? 'text-cyan-400' : 'text-muted-foreground'}`}>
